...
void Scene::init(int width, int height, std::string strTitle, bool bFullScreen){
	...
	boxCollider = std::make_shared<Box>(&shader);
    boxCollider->setColor(glm::vec4(1.0, 1.0, 1.0, 1.0));
	boxCollider->enableWireMode();

	sphereCollider = std::make_shared<Sphere>(&shader, 20, 20);
...
void Scene::prepareDepthScene(){
	terrain->setShader(&shaderDepth);

    esfera1->setShader(&shaderDepth);

    box1->setShader(&shaderDepth);
...
void Scene::renderAlphaScene(bool render){
    if(render)
...
void Scene::render(bool render){
    renderSolidScene();
	renderAlphaScene(render);
}
...
void Scene::run(){
    ...
    while (psi) {
		...
		// Settea la matriz de vista y projection al shader con textura
